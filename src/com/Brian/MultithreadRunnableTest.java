...
public class MultithreadRunnableTest {
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        //Runnable接口的对象传递给Thread类的构造函数。
        MultithreadRunnable r1 = new MultithreadRunnable("A");
        Thread t1 = new Thread(r1);

        t1.start();

        try {
            t1.join();// Waiting A finished before going to the B and C

        } catch (InterruptedException e) {
            e.printStackTrace();
        }

        new Thread(new MultithreadRunnable("B")).start();
        new Thread(new MultithreadRunnable("C")).start();
